...
For conceptual background and a discussion of when streaming materialized views are preferable to the incremental approach, see [DRAFT-streaming-materialized-views.md](DRAFT-streaming-materialized-views.md).
...
### Pipeline 1: Simple (`pipelines/simple-pipeline.mongodb.js`)
...
### Pipeline 2: With Escalation (`pipelines/escalation-pipeline.mongodb.js`)
...
Data plane commands (`db.*`) use a standard Atlas connection string. These cannot be mixed in a single session.
...
```js
load("streaming/pipelines/simple-pipeline.mongodb.js");
...
```js
load("streaming/watch.mongodb.js");
```
...
load("streaming/activity.mongodb.js");

// Includes priority escalations:
load("streaming/activity-escalation.mongodb.js");
```
...
```
streaming/
├── DRAFT-streaming-materialized-views.md   # Conceptual overview
├── README.md                               # This file
