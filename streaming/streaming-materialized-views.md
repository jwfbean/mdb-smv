...
**`$merge`** writes accumulated deltas to the view collection using an additive `whenMatched` pipeline rather than a replacement — this is what allows counts to accumulate correctly across windows. Because ASP provides at-least-once delivery guarantees, a window's output can be replayed after a processor restart. A naive `$add` is not idempotent and would double-count replayed windows. The correct pattern uses a `lastWindowStart` high-water mark: the delta is only applied if the incoming window is strictly newer than the last recorded one.

The `$group` stage must emit the window start timestamp alongside the count:
...
The `$merge` `whenMatched` pipeline then gates the delta on the high-water mark:
...
A replayed window fails the `$gt` check and is silently skipped. Sequential new windows pass and accumulate normally.
...
**Idempotency.** ASP provides at-least-once delivery guarantees — a window's output may be replayed after a processor restart. The high-water mark pattern described above (gating `$merge` on `lastWindowStart`) makes the merge idempotent against replays. Ensure the processor is fully stopped before dropping and recreating the view collection to avoid partial state.
